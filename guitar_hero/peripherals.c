...
void configButton(void){
    ...
    P7SEL &= ~(BIT0); //Setting IO
    P3SEL &=  ~(BIT6); //Setting IO
    P2SEL &= ~(BIT2); //Setting IO
    P7SEL &= ~(BIT4); //Setting IO
    ...
    P7OUT |= (BIT0); //Set-up to pull up
    P3OUT |= (BIT6); //Set-up to pull up
    P2OUT |= (BIT2); //Set-up to pull up
    P7OUT |= (BIT4); //Set-up to pull up
}

int press(){
    unsigned int S1 = 0x00;
    unsigned int S2 = 0x00;
    unsigned int S3 = 0x00;
    unsigned int S4 = 0x00;
...
void UserLEDon(char inbits){
    ...
    if ((inbits & BIT0) == 0x01){ //If 1.0 = 1, Light LED 1
        P1OUT |= BIT0;
    }
    if ((inbits & BIT1) == 0x02){ //If 4.7 = 1, Light LED 2
        P4OUT |= BIT7;
    }
}
