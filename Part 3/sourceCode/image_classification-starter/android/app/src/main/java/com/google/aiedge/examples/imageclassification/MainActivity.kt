            ...
            ApplicationTheme {
                BottomSheetScaffold(sheetPeekHeight = (90 + 20 * uiState.categories.size).dp,
                    sheetContent = {
                        ...
                        }, onInferenceModeSelected = { // Pass the callback to the ViewModel
...
//                    alignment = Alignment.CenterStart,
    ...
    fun BottomSheet(
        ...
        onInferenceModeSelected: (ImageClassificationHelper.Options.InferenceMode) -> Unit,
    ) {
        ...
        Column(
            ...
            // Inference Mode Selection UI
            Text(text = stringResource(id = R.string.inference_mode), fontWeight = FontWeight.SemiBold, fontSize = 16.sp, modifier = Modifier.padding(vertical = 10.dp))
            Row {
                RadioButton(
                    selected = uiState.setting.inferenceMode == ImageClassificationHelper.Options.InferenceMode.ON_DEVICE,
                    onClick = { onInferenceModeSelected(ImageClassificationHelper.Options.InferenceMode.ON_DEVICE) },
                    colors = RadioButtonDefaults.colors(selectedColor = MaterialTheme.colors.primary)
                )
                Text(
                    text = stringResource(id = R.string.on_device),
                ...
                RadioButton(
                    selected = uiState.setting.inferenceMode == ImageClassificationHelper.Options.InferenceMode.CLOUD,
                    onClick = { onInferenceModeSelected(ImageClassificationHelper.Options.InferenceMode.CLOUD) },
                    colors = RadioButtonDefaults.colors(selectedColor = MaterialTheme.colors.primary)
                )
                Text(
        ...
        Column(modifier = modifier) {
            radioOptions.forEach { option ->
                Row(
                    ...
                    Text(
                        modifier = Modifier.padding(start = 8.dp), text = option, fontSize = 15.sp
                    )
                }
            }
        }
