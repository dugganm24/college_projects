...
class ImageClassificationHelper(
    ...
    private val cloudInference: suspend (Bitmap) -> List<Pair<String, Float>>?
) {
    class Options(
        ...
        var inferenceMode: InferenceMode = DEFAULT_INFERENCE_MODE // Enum to specify whether to run inference on the device or in the cloud defaults to on device
    ) {
        ...
        companion object {
            val DEFAULT_INFERENCE_MODE = InferenceMode.ON_DEVICE // Default inference mode is on the device
        }
    }
    ...
    companion object {
        ...
        val DEFAULT_INFERENCE_MODE = Options.InferenceMode.ON_DEVICE // Default inference mode is on the device
    ...
    suspend fun initClassifier() {
        interpreter = try {
            ...
            val options = Interpreter.Options().apply {
                numThreads = this@ImageClassificationHelper.options.threadCount
                useNNAPI = this@ImageClassificationHelper.options.delegate == Delegate.NNAPI
            }
    ...
    suspend fun classify(bitmap: Bitmap, rotationDegrees: Int) {
        // Check the inference mode and call the appropriate function.
        if (options.inferenceMode == Options.InferenceMode.ON_DEVICE) {
            performOnDeviceInference(bitmap, rotationDegrees) // Call the on-device inference function.
        } else {
            performCloudInference(bitmap) // Call the cloud inference function.
        }
    }

    // Function to perform on-device inference using TensorFlow Lite.
    private suspend fun performOnDeviceInference(bitmap: Bitmap, rotationDegrees: Int) {
        try {
            ...
            Log.i(TAG, "On-device image classification error occurred: ${e.message}")
            _error.emit(e)
        }
    }

    // Function to perform cloud inference using the injected lambda.
    private suspend fun performCloudInference(bitmap: Bitmap) {
        try {
            withContext(Dispatchers.IO) {
                val startTime = SystemClock.uptimeMillis()
                val cloudResults = cloudInference(bitmap) // Call the injected cloud inference function.

                if (cloudResults != null) {
                    val thresholdedCloudResults = cloudResults.map { (label, score) ->
                        if (score < options.probabilityThreshold) {
                            label to 0f // Zero out scores below the threshold
                        } else {
                            label to score
                        }
                    }

                    val categories = thresholdedCloudResults
                    ...
                    Log.e(TAG, "Cloud inference failed or returned null results.")
                    _error.emit(IOException("Cloud inference failed or returned null results."))
                }
            }
        } catch (e: Exception) {
