...
// This class is a singleton that implements the TaskRepository interface
@Singleton
class DefaultTaskRepository @Inject constructor(
    ...
    // Creates a new task in the local data source and saves it to the network data source
    override suspend fun createTask(title: String, description: String): String {
        ...
        saveTasksToNetwork() // After updating local data a network sync may need to occur
        return taskId
    }

    // Updates a task in the local data source and saves it to the network data source
    ...
    // Gets a list of tasks from the local data source and maps them to the external model
    ...
    // Refreshes task by ID from network
    override suspend fun refreshTask(taskId: String) {
        refresh()
    }

    // Retrieves stream of specific task
    ...
    // Marks task as completed in local data source and saves to network
    override suspend fun completeTask(taskId: String) {
        localDataSource.updateCompleted(taskId = taskId, completed = true)
        saveTasksToNetwork()
    }

    // Marks task as active in local data source and saves to network
    ...
    // Refreshes local data from network
